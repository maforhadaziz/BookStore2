...
const Home = () => {
  ...
  const [stats, setStats] = useState({ totalBooks: 0, totalCategories: 8, totalUsers: 0 });
  ...
  console.log('Debug localStorage - isAdmin:', localStorage.getItem('isAdmin'));
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/api/books`)
      .then(res => {
        ...
        // Check lastViewedBook validity for Continue Reading
        const lastViewed = localStorage.getItem('lastViewedBook');
        if (lastViewed) {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/api/books/trending?limit=3`)
      .then(res => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/api/books/analytics/total-users`)
      .then(res => {
        setStats(prev => ({ 
          ...prev, 
      ...
      .catch(() => {
        // If the endpoint fails, set default values
        setStats(prev => ({ 
          ...prev, 
          totalUsers: 0 
        }));
          ...
          <div className="hero-text">
            {isAuthenticated && (
              <div className="user-greeting">
                <h2>Hello, {userName || userEmail || 'Reader'}! 👋</h2>
              </div>
            )}
          ...
          <div className="continue-reading-card" onClick={() => handleBookClick(continueReading)}>
            <div className="continue-book-cover">
              {continueReading.coverImageFileName ? (
                <img 
                  src={`${process.env.REACT_APP_API_URL}/api/uploads/${continueReading.coverImageFileName}`} 
              ...
              <div className="trending-book-card" key={book._id}>
                ...
                <div className="trending-book-cover">
                  {book.coverImageFileName ? (
                    <img 
                      src={`${process.env.REACT_APP_API_URL}/api/uploads/${book.coverImageFileName}`} 
                      alt={book.title} 
                    />
                  ) : (
